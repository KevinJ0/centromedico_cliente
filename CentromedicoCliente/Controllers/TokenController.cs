...
namespace CentromedicoCliente.Controllers
{
    ...
    public class TokenController : Controller
    {
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> RegisterAsync([FromBody] RegisterDTO formdata)
        {
            try
            {
                var result = await _tokenSvc.RegisterAsync(formdata);

                if (result)
                    return await _tokenSvc.GenerateNewToken(new TokenRequestDTO
                    {
                        UserCredential = formdata.Email,
                        Password = formdata.Password
                    }, mobile: false);
                else
                      throw new BadRequestException("Ha ocurrido un error al tratar de registrar al usuario.");
